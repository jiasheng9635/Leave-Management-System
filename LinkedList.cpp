...
bool LinkedList<ItemType>::sortInsert(const ItemType& newEntry)
{
    ...
    if (itemCount==0)
    {
        //cout << newEntry << " add first" << endl;
        newNodePtr->setNext(headPtr);
        headPtr = newNodePtr;
    }
    else if ( newEntry <= (headPtr->getItem()) )
    {
        //cout << newEntry << " add front" << endl;
        newNodePtr->setNext(headPtr);
        headPtr = newNodePtr;
    }
    else
    {
        ...
        {
            Node<ItemType>* prevPtr = getNodeAt(i);
            Node<ItemType>* curPtr = prevPtr->getNext();
            if( (newEntry >= (prevPtr->getItem())) && (prevPtr->getNext()==nullptr) )
            {
                //cout << newEntry << " add back" << endl;
                newNodePtr->setNext(nullptr);
                prevPtr->setNext(newNodePtr);
            }
            else if ((newEntry >= prevPtr->getItem()) && (newEntry <= curPtr->getItem()))
            {
                //cout << newEntry << " add middle" << endl;
                // Insert new node after node to which prevPtr points
...
template<class ItemType>
ostream& operator<< (ostream &o, LinkedList<ItemType> &i)
{
    if (i.isEmpty())
    {
        o<<"List is empty";
    ...
    {
        Node<ItemType>* ptr = i.headPtr;
        while (ptr!=nullptr)
        {
            ItemType s = ptr->getItem();
            o << s << "\n";
            ptr = ptr->getNext();
        }
    }
